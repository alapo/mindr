...
#' mm(from = input, type = 'dir', show_files = TRUE)
#' ### directory -> mm
...
# input is dir ---------------------------------
    return(tree(from = from, to = to, root = root, show_files = show_files,
...
tree <- function(from = '.',
    ...
    if(!is.null(to)) {
      ...
      to_dir <- dirname(to)
      to_ext <- get_filename_ext(to_name)
      header <- dir2(path = from, output = to_ext, savefile = TRUE, savefilename = to, dir_files = show_files)
    }
  header <- dir2(path = from, output = 'md', savefile = FALSE, dir_files = show_files)
  header <- paste0('#', header)
