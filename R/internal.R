...
dir2 <- function(path = getwd(),
  ...
  if (dir.exists(path)) {
    ...
    # char_reduntant <- readLines(system.file('resource/reduntant.txt', package = 'mindr'), encoding = 'UTF-8')
    # for(i in c('\033\\[01;34m', '\033\\[00m')) mytree <- gsub(i, '', mytree)
    ...
    if(dir_files & os == 'Windows'){
      ...
      # md[loc_files] <- gsub(pattern = '    ([^ ]+)', paste0(non_ascii[2], '  \\1'), md[loc_files])
      # md[loc_files] <- gsub(paste0(non_ascii[2], '  '), '#', md[loc_files])
    ...
    # md <- gsub(pattern = paste0(non_ascii[2], '  '), '#', md)
    md <- gsub(pattern = '    ', '#', md)
